...
public class ListarSinistrosUseCase {
    ...
    public PageResponseDTO<SinistroResponseDTO> executar(
        ...
        if (!"ANALISTA".equals(usuarioPapel) && !"GESTOR".equals(usuarioPapel) && !"ADMIN".equals(usuarioPapel)) {
            throw new AcessoNegadoException("Acesso negado. Apenas analistas, gestores ou administradores podem listar sinistros.");
        }

        // ANALISTA só pode ver os sinistros atribuídos a si mesmo
        if ("ANALISTA".equals(usuarioPapel)) {
            analistaId = UUID.fromString(usuarioId);
        }
